...
def Dataplot(parameter,T,df2,my_xaxis,my_yaxis,plot_name):
	...
	B = df2['B'].values
	Rxy= twobandfit_T(parameter,df2[my_xaxis],T)
	# 原点相交
	'''
	fig = plt.gca()
	...
	plt.plot(df2[my_xaxis],Rxy,label = 'Rxy Theory')

	plt.text(2.2, 0.95, 'Temperature:'+str(round(T,4)), fontsize=10)
	plt.text(2.2, 0.9, 'np:'+str(round(parameter[0],4)), fontsize=10)
	plt.text(2.2, 0.85, 'up:'+str(round(parameter[1],4)), fontsize=10)
	plt.text(2.2, 0.8, 'ne:'+str(round(parameter[2],4)), fontsize=10)
	plt.text(2.2, 0.75, 'up:'+str(round(parameter[3],4)), fontsize=10)
